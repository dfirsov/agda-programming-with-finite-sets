...
list-div : {X : Set} (a b : ℕ) → (xs : List X) 
  → a ≤n b → Σ[ xs' ∈ List X ] take b xs ≡ take a xs ++ xs'
...
mapinloc : ∀{a} {X : Set a} (x : X) (xs ys : List X) → x ∈ xs → xs ≡ ys → x ∈ ys
mapinloc x xs .xs p1 refl = p1
...
... | ass , ps  = there ass , there (mapinloc _ _ _ ps (sym (mapmap f (λ { (e , p) → e , there p })  (lft as))))
